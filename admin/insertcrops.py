...
try:
    ...
    if(dbcon):
        ...
        if imagename.filename:
            ...
            open("FileUpload/crops/"+fn,'wb').write(imagename.file.read())
            file_name=imagename.filename
            iquery="insert into tbl_crops(name,image,grows,season,compost) values('%s','%s','%s','%s','%s')"%(vname,file_name,grows,season,compost)
            res=cursor.execute(iquery)
            if(res==1):
                dbcon.commit()
                print("<script>alert('Addedd Successfully');location.href='viewcrops.py';</script>")
            else:
                dbcon.commit()
                print("<script>alert('Error in Adding');location.href='viewcrops.py';</script>")
