...
try:
    ...
    if(dbcon):
        ...
        if imagename.filename:
            ...
            open("FileUpload/Trees/"+fn,'wb').write(imagename.file.read())
            file_name=imagename.filename
            query="insert into tbl_trees(name,image,grows,season,compost) values('%s','%s','%s','%s','%s')"%(vname,file_name,grows,season,compost)
            res=cursor.execute(iquery)
            if(res==1):
                dbcon.commit()
                print("<script>alert('Addedd Successfully');location.href='viewtrees.py';</script>")
            else:
                dbcon.commit()
                print("<script>alert('Error in Adding');location.href='viewtrees.py';</script>")
